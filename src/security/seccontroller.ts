...
export class SecController {
	...
	public static async login(req: Request, res: Response): Promise<void> {
		...
		try {
			...
			if (!await SecUtils.compareToHash(password, result.data.document.password)) {
				res.send({ status: 'error', data: 'Invalid login' });
				return;
			}
			const token = SecUtils.getToken({ _id: result.data.document._id, username: userName });
	...
	public static async changePwd(req: Request, res: Response): Promise<void> {
		...
		try {
			...
			if (result.data.matchedCount>0)
				...
				res.send({ status: 'error', data: 'Error updating password, record not found' });
		}catch(e){
			console.error(e);
			res.send({ status: 'error', data: e });
		}
